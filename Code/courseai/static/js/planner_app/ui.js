// TODO: 21/08/2018 SetupDegreeRequirements, updateMMS, updateProgress.
...
function addCourse(code, title, session, position, update_recommendations = true) {
    ...
    const sem = 'Semester ' + session.slice(4); // TODO: Fix for Summer Sessions
...
function removeCourse(session, position, update_recommendations = true) {
    ...
    PLAN.removeWarning('CourseForceAdded', slot['code']);
...
async function mms_add(code, year) {
    const mms = getMMSOffering(code, year);
...
setupDegreeRequirements(starting_degree);
...
$('.filter-delete').click(deleteFilter);    // TODO: Check if this can be removed.
...
function makeCourseDraggable(item, code, year) {
    item.draggable({
        ...
        start: function (event, ui) {
            ...
            const offering = getCourseOffering(code, year); // TODO: Check if this is running on the main thread.
            highlightInvalidSessions(offering);
        },
        stop: function (event, ui) {
...
function highlightInvalidSessions(offering) {
    ...
    for (const session of PLAN.sessions) {
        ...
        const offered = offering_sessions.includes(session.slice(-1));
        if (!checked.sat) {
...
function setupDegreeRequirements(degree) {
    const required = degree.rules.required;
    ...
    function createMMSListSection(type, title, mms) {
        ...
        for (let code of mms) {
            ...
            mms_to_retrieve[{'code': code, 'year': year}] = function (mms) {
                title_node.text(mms.name);
            };
            let item = $(
        ...
        if (type === "required_m/m/s") {
            for (let code of required[type]) {
                ...
                mms_to_retrieve[{'code': code, 'year': year}] = function (mms) {
                    ...
                    if (mms_to_display.includes(code)) mms_add(code, year);
    ...
    $.when.apply($, courseListRequests).then(function () {
        batchCourseTitles(titles_fill_nodes);
