...
function resetPlan(){SAVER.disableSaving();clearPlanner();setupPlanner(!0)}function updateColorLegend(){var a=$("#color-coding-section");a.empty();for(var b=0;b<COLOR_CLASSES.length;b++){var c=COLOR_CLASSES[b];if(c in legendMappings){var d=$('<div class="color-coding-row"/>'),e=$('<div class="color-coding-box"/>'),f=$('<span class="color-coding-text"/>');e.css("background-color",colorMappings[c]);f.text(legendMappings[c]);d.append(e);d.append(f);a.append(d)}}}
function addLinearGradient(a,b){for(var c=["-webkit-","-moz-","-o-","-ms-"],d=0;d<c.length;d++){for(var e=c[d]+"linear-gradient(135deg",f=100/a.length,g=0;g<a.length;g++){var n=g==a.length-1?100-f:(g+1)*f;0<g&&g<a.length-1&&(e+=","+colorMappings[a[g]]+" "+f*g+"%");e+=","+colorMappings[a[g]]+" "+n+"%"}e+=")";b.css("background",e)}}
